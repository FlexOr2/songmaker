...
function routeAwareCowriterSettings(overrides: Record<string, unknown> = {}) {
	return {
		...
		models_by_provider: { claude: ['claude-cli'] },
		models_errors: {},
		models_sources: {},
		provider_routes: { claude: 'cli' as const },
...
describe('admin models tab', () => {
	...
	it('shows each route state, redacts API keys, and changes the model catalog with the selected route', async () => {
		...
		expect(requireElement<HTMLSelectElement>(cowriter, '#cowriter-model').value).toBe('claude-cli');

		buttonNamed(cowriter, 'API').click();
		await tick();

		expect(requireElement<HTMLSelectElement>(cowriter, '#cowriter-model').value).toBe('claude-api');
	...
	it('shows a selected broken route as a blocked turn without silently falling back', async () => {
		...
		expect(requireElement<HTMLSelectElement>(cowriter, '#cowriter-model').disabled).toBe(true);
	});
