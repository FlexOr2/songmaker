...
describe('LibraryRow', () => {
	...
	describe.skip('retired row filter', () => {
		const LEGACY_ROW_FILTER_EMPTY = 'Nothing matches';
		...
		it('shows a short line when nothing matches the filter', async () => {
			const root = await render({ kind: 'album', id: 'a-1' });
			expect(root.textContent).not.toContain(LEGACY_ROW_FILTER_EMPTY);

			await typeFilter(root, 'zzz-nothing-matches');

			expect(root.textContent).toContain(LEGACY_ROW_FILTER_EMPTY);
		});
		...
		it('clears the filter on Escape', async () => {
			...
			filterField(root).dispatchEvent(
