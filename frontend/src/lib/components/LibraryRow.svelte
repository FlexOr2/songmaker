<script lang="ts">
	...
	const visibleTileKey = $derived(JSON.stringify(tiles.map((tile) => tile.id)));
	...
	// and whenever the set of tiles changes, which can shift the open tile
	// off-centre even though it never moved in the underlying list.
	$effect(() => {
		const row = rowEl;
