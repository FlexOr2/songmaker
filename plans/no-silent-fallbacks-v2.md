...
| W2 | Pydantic discriminated union for `generation_params` | ✅ **Done** | New `api_models/generation_params.py` (`BaseGenerationParams` / `StoredGenerationParams` / `RepaintTaskParams` / `CoverTaskParams`, all `extra="forbid"`). SQLAlchemy `@validates` on `Version.generation_params`, `Generation.generation_params`, `GenerationPreset.params`. `build_ace_config` rewritten to take typed layers, `_apply_task_overrides` split into typed `_apply_repaint_overrides` / `_apply_cover_overrides`. arq dict transport replaced with typed models (validated in `MusicWorker.generate`). `SongMeta` got first-class `bpm`/`audio_duration`/`key_scale`/`vocal_language` fields and a typed `generation_params`. Scoring `.get()` calls in `bpm_accuracy.py` / `text_accuracy.py` are gone. Migration script `scripts/migrate_generation_params.py` lands with `--dry-run`/`--fix`. 1336 tests pass. |
| W3 | Kill the 20 silent-fallback smell sites | ✅ **Done** | New `WorkerProtocolError` (subclass of `WorkerTaskFailed`) raised in [scheduler.py:298,306,329,337](../src/songmaker_cli/scheduler.py#L298) for missing `result`/`error` SSE fields; empty `error` field logs a warning and propagates. `INITIAL_TRACK_NUMBER` / `INITIAL_GENERATION_NUMBER` / `INITIAL_PLAYLIST_POSITION` named constants in the matching `db/queries/` modules. `settings_api._build_model_response` now raises `RuntimeError` if a registered model is missing from `_BUILTIN_DEFAULTS` / `ACESTEP_PROFILES` (was silently returning `{}`). `health_api._format_prometheus` worker-metric params are now required (caller always passes them). `api_models/playlists.py` `_playlist_entries` helper logs a warning if SQLAlchemy returns `None` for the relationship. `api_helpers._job_type_rate_limits` was already raising on unknown job types — no change needed. LOW sites left alone (CLAUDE.md "no inline comments"); they'll be allowlisted in W5's CI smell-checker. 1348 tests pass. |
| W4 | Tighten `Optional` types in interior signatures | ✅ **Done** | Required-timestamp fields (`UserResponse.created_at`, `SessionResponse.created_at/expires_at`, `LoginAttemptResponse.attempted_at`, `AuditLogResponse.created_at`, `GenerationResponse.created_at`, `VersionResponse.created_at`, `SongSummaryResponse.created_at`) dropped `\| None` and the corresponding `from_orm` methods dropped `if x else None`. `PresetResponse` / `ChatMessageResponse` `from_orm` already had `str` types but kept defensive `else ""` fallbacks — those are gone too. `_best_generation` typed `list[Generation] -> Generation \| None`. `run_generation_job` `db_factory`/`audio_dir`/`data_dir`/`redis` are now required positional kwargs (no more `\| None = None` + runtime asserts). `SongSummaryResponse.key_scale` flipped from `str = ""` to `str \| None = None` for uniformity (None means missing across the board). `gen_params_to_dict` renamed to `gen_params_to_json` since it serializes to JSON-storable shape; all 5 callers updated. 1348 tests pass with no test changes needed (the tightening was zero-regression). |
| W5 | Tests + CI smell-checker | ✅ **Done** | New `scripts/check_no_silent_fallbacks.py` with 6 rules: `env-read-outside-settings`, `next-iter-fallback`, `dict-get-domain-fallback`, `dict-any-in-signature`, `optional-on-default-utcnow-column`, `engine-isolation-violation`. Per-rule allowlists for the few legitimate exceptions (`db/migrations/env.py`, `audiobox_aesthetics.py`, `claude/provider.py`, the engine `settings.py` modules, `acestep_worker/task_store.py`). Wired into `.github/workflows/ci.yml` between `ruff` and the type-sync check. 11 unit tests in `tests/test_check_no_silent_fallbacks.py` cover every rule plus the clean-codebase invariant. No `xfail` cleanup needed — the W1–W4 commits each shipped with full green suites. Stray missed `PlaylistResponse.created_at: str \| None` from W4 caught and tightened. 1359 tests pass. |
