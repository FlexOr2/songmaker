...
- **One click rule** (operator decision A, 2026-08-23): a click opens the object, ▶ plays only. A song row opens the editor; its ▶ plays the pick (greyed out without one). A take row — editor takes list, playlist interior, rail playlist rows, queue — plays the take **and** opens Now Playing on "This take", which on desktop is the docked panel, so no place is lost; "Go to song" stays the bridge into the editor. Take rows show version/duration/★, song rows title/take count. Share pages stay play-only.
