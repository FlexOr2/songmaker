...
**Status:** Proposed
**Date:** 2026-04-09
**Context:** Brutal architecture review of the codebase identified 12 findings. This document records each one, its current status (covered by another plan / quick-win ready / deferred), and the decisions needed to act on it.
**Related plans:**
- [no-silent-fallbacks-v2.md](no-silent-fallbacks-v2.md) — covers B2, B7, B12 (and parts of B1's prep)
- [jobs-module-split.md](jobs-module-split.md) — pre-existing plan, now sequenced after no-silent-fallbacks-v2

## Quick reference

| # | Finding | Status | Owner action |
|---|---|---|---|
| B1 | `jobs.py` 925-line god module | **Deferred** — execute after no-silent-fallbacks-v2 | Sequencing locked in |
| B2 | Hardcoded `task_type` strings | **Covered** by no-silent-fallbacks-v2 W2 | None — replaced by Pydantic `Literal` |
| B3 | Scorer subprocess Pipe race | **Quick-win ready** | Lock the Pipe in pending PR |
| B4 | CI excludes scoring coverage | **Accepted** — note it, move on | Add note to CI config + CLAUDE.md |
| B5 | `worker_base.py` is not a base class | **Quick-win ready** | Full `WorkerBase` class refactor |
| B6 | `Generation.version_id` missing index | **Quick-win ready** | One line + Alembic migration |
| B7 | Import-time singletons | **Covered** by no-silent-fallbacks-v2 W1 | None |
| B8 | Stuck-`QUEUED` jobs never recovered | **Quick-win ready** | `QUEUE_MAX_AGE_SECONDS = 600` + recovery rule |
| B9 | Backpressure invisible to UI | **Plan needed** — design locked in below | Implement after no-silent-fallbacks-v2 |
| B10 | PID-based stale detection | **Quick-win ready** | Drop PID fallback + heartbeat NOT NULL migration |
| B11 | Stale `plans/` directory | **Quick-win ready** | Status headers + archive folder |
| B12 | Configuration scattered | **Covered** by no-silent-fallbacks-v2 W1 | None |

## Sequencing

```
1. Quick-wins PR        (B3, B5, B6, B10, B11 + B4 note)
...
## B2 — Hardcoded `task_type` strings ✓ COVERED
...
**Coverage:** [no-silent-fallbacks-v2.md](no-silent-fallbacks-v2.md) Workstream 2 introduces a Pydantic discriminated union with `Literal["text2music"|"repaint"|"cover"]`. Type-safe, no separate enum needed, the type system catches typos.

**Action:** None. Verify the string compares are gone after W2 lands.

---

## B3 — Scorer subprocess Pipe race at `SCORING_MAX_JOBS > 1`
...
## B4 — CI excludes the entire scoring pipeline (ACCEPTED)
...
## B5 — `worker_base.py` is not a base class
...
## B6 — `Generation.version_id` missing index
...
## B7 — Import-time singletons (REDIS_URL, CLAUDE_*_MODEL) ✓ COVERED
...
**Action:** None. Verify the entries are gone after W1 lands.
...
## B10 — PID-based stale detection collides with PID reuse
...
## B12 — Configuration scattered across 22 files ✓ COVERED
...
**Action:** None. Verify after W1 lands.
