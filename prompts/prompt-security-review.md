...
- **Claude CLI tool allowlist**: `provider.py` builds the session with `--tools ""`, `--setting-sources ""`, `--strict-mcp-config` and `--allowedTools mcp__songmaker__*`, and `verify_cli_tool_surface()` refuses a binary that announces anything else. No hand-kept tool-name list remains. Section 4 should still assess prompt-based tool invocation bypasses.
