...
- **Scorer model caches are module-level globals** (`_whisper_model` in `text_accuracy.py`, `_predictor` in `audiobox_aesthetics.py`). These live in the scorer subprocess, so leaks are contained and cleaned up on subprocess kill. Still prevents `pytest-xdist` parallel execution for scoring tests.
