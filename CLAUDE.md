...
- **Claude CLI bind mounts in `docker-compose.yml`** never expose an operator profile. The web container owns writable `.claude`, `.grok`, and `.codex` profiles; the scoring worker owns `.claude` only. Each mounts the corresponding CLI binary and redacted credential mirror read-only with `create_host_path: false`; the scoring worker deliberately has no Grok or Codex login. Claude creates `~/.claude.json` in its own profile when needed. `ANTHROPIC_API_KEY` enables Claude's judge and catalog SDK path, but not the co-writer, which needs the CLI with Songmaker's MCP tools; retain the web Claude CLI mirror for co-writer turns. See `docs/security.md`, "Agent-CLI Mounts".
