...
- **Scorer model caches are module-level globals** (`_whisper_model` in `text_accuracy.py`, `_predictor` in `audiobox_aesthetics.py`). This prevents `pytest-xdist` parallel execution for scoring tests. Refactoring to injectable caches is straightforward (~4 hours) but low priority unless parallel test execution is needed.
