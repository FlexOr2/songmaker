...
- **Claude CLI bind mounts in `docker-compose.yml`** are a temporary workaround for using a Max subscription instead of an API key. The same three mounts (`~/.local/bin/claude`, `~/.claude`, `~/.claude.json`) appear on both `songmaker-web` (co-writer chat) and `songmaker-scoring-worker` (`lyrical_coherence` judge, issue #182 — without them the judge raised `UnavailableError` on every score, since `ANTHROPIC_API_KEY` is unset by default and `call_claude()` fell through to a CLI that wasn't there). They give each container access to the host's CLI binary and credentials. When switching to `ANTHROPIC_API_KEY`, remove all three mounts from both services — the provider auto-prefers the API key over CLI.
...
- **No Claude CLI call carrying song content runs unverified — the gate sits in the call paths, not in each caller, and not in the command builders themselves (the builders just format flags; they check nothing).** `_build_cli_cmd()` (tool-free: the legacy `/songs/{id}/chat` endpoint and the lyrical-coherence judge) is reached only through `_call_cli()`/`_acall_cli()` in `claude/provider.py`, and those two call `verify_no_builtin_cli_tools()`/`averify_no_builtin_cli_tools()` themselves before building anything — so a future caller of `call_claude()`/`acall_claude()` inherits the gate automatically instead of needing to remember it. `_build_mcp_cli_cmd()` (the co-writer, MCP tools attached) is reached only through the two MCP entry points (`acall_claude_with_mcp`/`acall_claude_with_mcp_stream`), which call `verify_cli_tool_surface()` themselves the same way — a separate gate, not routed through `_call_cli()`/`_acall_cli()`. Both command lines apply `--tools ""`, `--setting-sources ""`, `--strict-mcp-config`, and `--disable-slash-commands` (removing the CLI's built-ins, the mounted `~/.claude/settings.json`, foreign MCP servers, and slash-command/skill dispatch respectively). `--permission-mode bypassPermissions` is gone from both.
  The two gates read the tool list the CLI itself reports on startup (`--output-format stream-json --verbose`, first init-event line, `subtype` checked too) on a cache **miss** only — a cache hit returns the remembered verdict without starting a CLI session at all. The co-writer's MCP-attached probe expects **exactly** the eleven `mcp__songmaker__*` tools — a hand-maintained literal tuple in `provider.py` (not imported from `mcp_server/server.py`, to avoid pulling the `mcp` package into the scoring-worker container), kept honest by a dedicated test that compares it against that module's real registration; set equality, not a prefix check, so a tool going missing is caught the same as an extra one. The no-MCP probe, used by every tool-free call, expects **no tool at all**, since that call line never attaches ours either — and never attaches `--mcp-config`, so it needs neither a reachable database (registering and listing MCP tools touches no DB — only a tool *call* does) nor the `mcp` extra, and stays importable in the scoring-worker container, which has neither (`tests/test_packaging_boundary.py`). Both also require the reported slash-command list to be empty.
  A verdict — a real answer about this exact binary build's tool surface, mismatch or clean — is cached per resolved build with no expiry, because it does not change until the build does (a self-update is a different build, re-probed). A probe **failure** — anything that kept the answer from being trustworthy, including a timed-out probe, unparseable output, or (co-writer path) the songmaker MCP server failing to connect, which reports the same empty tool list a clean but genuinely tool-free CLI would — is a different thing and is cached separately, for only `CLAUDE_CLI_TOOL_SURFACE_FAILURE_CACHE_SECONDS`: a real drift stays refused until a new build lands, but an infra hiccup does not lock the co-writer out until the process restarts. Concurrent cold-cache callers for the same key (binary build × expectation) single-flight: the first probes while holding a per-key lock (`asyncio.Lock` for the two async entry points, `threading.Lock` for `_call_cli`'s sync path), the rest wait on that lock and reuse its answer rather than each spawning a CLI process — proven with genuine concurrency (`asyncio.gather`, real threads), not sequential calls. Sync and async callers for the *same* key do not exclude each other (a thread lock cannot be awaited without blocking the loop, an asyncio lock cannot be waited on without one) — an accepted, narrow gap given the only key both domains ever share is the no-MCP one. Because the lock is held across the whole probe, its cleanup is bounded too: after a process ignores SIGTERM and then SIGKILL (which it cannot ignore — normal exit is immediate), the final wait for it to actually exit gives up after `CLAUDE_CLI_ZOMBIE_REAP_TIMEOUT_SECONDS` rather than holding the lock (and every later caller of that key) forever on the pathological case of a process stuck in an uninterruptible kernel sleep; the process is handed to a background task/thread that keeps waiting for it so it is still reaped, and both the giving-up and the eventual reap are logged. The gate resolves the CLI's symlink once and runs that literal path for both the probe and the real turn, so what was checked is what executes even if the mounted `claude` binary self-updates in between. The probe cannot guarantee zero API cost: the CLI's own `--max-budget-usd` was checked live and only aborts a session after a call completes, not before one starts.
