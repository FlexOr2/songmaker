...
- **Scoring modules excluded from CI coverage.** All seven scorers in `scoring/` are listed in `.coveragerc-ci` `omit`. Reason: the CI image doesn't ship faster-whisper / audiobox-aesthetics / librosa model weights, and adding them blows up image size and runtime for a single-developer project. Local coverage runs include them. See B4 in `plans/architecture-review-findings.md`.
