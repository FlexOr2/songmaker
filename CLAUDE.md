...
- **`AceStepManager.switch_model` is single-user / single-GPU debt.** Dynamic in-process model swapping (stop subprocess → load new model → wait for health) blocks the worker for ~30–90s per switch. Acceptable for single-user operation where switches are infrequent. Under multi-user load with mixed model preferences it thrashes. The state machine (`_switch_lock`, `_active_config_path`, verification + rollback in `acestep_manager.py`) exists only because we run on one GPU. **Do not extend it** — every line added is throwaway. When a second GPU appears or sustained multi-user load arrives, build `plans/multi-model-routing.md` and delete `switch_model` along with its lock/persistence machinery in the same PR. The two approaches are mutually exclusive, never run them in parallel.
