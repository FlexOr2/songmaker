...
- **No Claude CLI call carrying song content runs unverified — the gate lives in the two functions that build the command line, not in each caller.** Every such call funnels through exactly one of two internal functions in `claude/provider.py`: `_build_mcp_cli_cmd()` (the co-writer, MCP tools attached) or `_build_cli_cmd()` (the legacy `/songs/{id}/chat` endpoint and the lyrical-coherence judge, no tools at all) — both apply `--tools ""`, `--setting-sources ""`, `--strict-mcp-config`, and `--disable-slash-commands` (removing the CLI's built-ins, the mounted `~/.claude/settings.json`, foreign MCP servers, and slash-command/skill dispatch respectively), and both are reached only through `_call_cli()`/`_acall_cli()`, which call the verification gate themselves before building anything — so a future caller of `call_claude()`/`acall_claude()` inherits the gate automatically instead of needing to remember it. `--permission-mode bypassPermissions` is gone from both.
  Two gates share one probe mechanism (`verify_cli_tool_surface()` / `verify_no_builtin_cli_tools()` and its async twin), reading the tool list the CLI itself reports on startup (`--output-format stream-json --verbose`, first init-event line, `subtype` checked too) rather than trusting a hand-kept list: the co-writer's MCP-attached probe expects **exactly** the eleven `mcp__songmaker__*` tools in `mcp_server/server.py` (set equality, not a prefix check — a tool going missing is caught same as an extra one); the other probe, used by every tool-free call, expects **no tool at all**, since that call line never attaches ours either. Both also require the reported slash-command list to be empty. A mismatch on either raises `CliToolSurfaceError` and only that call is refused, not the whole server. Verdicts are cached per resolved binary build (`--tools ""` and friends don't change that identity, so a self-update is re-probed), and a probe **failure** is also cached, briefly (`CLAUDE_CLI_TOOL_SURFACE_FAILURE_CACHE_SECONDS`), so a struggling CLI does not make every concurrent caller each wait out the probe timeout. The gate resolves the CLI's symlink once and runs that literal path for both the probe and the real turn, so what was checked is what executes even if the mounted `claude` binary self-updates in between. The probe cannot guarantee zero API cost: the CLI's own `--max-budget-usd` was checked live and only aborts a session after a call completes, not before one starts.
  The scoring-worker container has no boot-time check and does not install the `mcp` extra — the MCP-attached probe would always fail there (a missing MCP server reports zero tools, not eleven, confirmed live), so the no-MCP probe deliberately never touches `--mcp-config` and stays importable without the `mcp` package (`tests/test_packaging_boundary.py`). Replaced the former hand-maintained denylist, which was already missing eight tools (`Task`, `DesignSync`, `ListAgents`, `Monitor`, `PushNotification`, `ReportFindings`, `ScheduleWakeup`, `Workflow`) against the installed 2.1.257 CLI's 26.
