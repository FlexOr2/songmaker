...
- **No Claude CLI call carrying song content runs unverified — the gate sits in the call paths, not in each caller, and not in the command builders themselves (the builders just format flags; they check nothing).** `_build_cli_cmd()` (tool-free: the legacy `/songs/{id}/chat` endpoint and the lyrical-coherence judge) is reached only through `_call_cli()`/`_acall_cli()` in `claude/provider.py`, and those two call `verify_no_builtin_cli_tools()`/`averify_no_builtin_cli_tools()` themselves before building anything — so a future caller of `call_claude()`/`acall_claude()` inherits the gate automatically instead of needing to remember it. `_build_mcp_cli_cmd()` (the co-writer, MCP tools attached) is reached only through the two MCP entry points (`acall_claude_with_mcp`/`acall_claude_with_mcp_stream`), which call `verify_cli_tool_surface()` themselves the same way — a separate gate, not routed through `_call_cli()`/`_acall_cli()`. Both command lines apply `--tools ""`, `--setting-sources ""`, `--strict-mcp-config`, and `--disable-slash-commands` (removing the CLI's built-ins, any profile `~/.claude/settings.json`, foreign MCP servers, and slash-command/skill dispatch respectively). `--permission-mode bypassPermissions` is gone from both.
  ...
  The probe itself runs under one overall deadline covering process start, stdin write,
  and stdout read together — not a separate timeout per step that can add up to an
  unknown total (spawn and the pipe write used to have none at all). The async gate
  does not spawn via `asyncio.create_subprocess_exec` directly: that still runs the
  underlying `fork()`/`exec()` synchronously on whichever thread calls it, including
  the event loop's own, so a stuck spawn could keep the loop from ever running the
  timer meant to enforce the deadline. Both gates instead run the one real probe body
  (`_probe_cli_surface_sync`) on a worker thread via `asyncio.to_thread`, so a hung
  `Popen()` only blocks its own thread, never the caller waiting on it with a real
  deadline. Only cleanup (SIGTERM grace, then the post-SIGKILL wait) has its own
  small, separate bound, `CLAUDE_CLI_ZOMBIE_REAP_TIMEOUT_SECONDS`: cleanup must
  always be attempted regardless of how much of the answer budget is left. The caller
  can therefore observe the probe timeout plus that cleanup margin after its answer
  budget expires. A process that survives even that is a zombie — handed to a
  background reaper capped and deduplicated by PID
  (`CLAUDE_CLI_MAX_CONCURRENT_ZOMBIE_REAPERS`, small on purpose) so a run of bad
  probes cannot grow an unbounded pool of waiting tasks/threads (measured by the
  review: ~225/hour at one per immediately-invalid probe, uncapped); once the pool is
  already full, a *new* probe refuses to even start (the OS does not reap an abandoned
  child of a still-live parent on its own, so a spawned-but-untracked ninth zombie
  would otherwise just sit in the process table). The app's shutdown (`server.py`'s
  lifespan) cancels every outstanding background async reaper the same way it already
  cancels its other background loops.
