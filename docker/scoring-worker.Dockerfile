...
RUN uv sync --frozen --no-dev --no-install-project --extra server --extra scoring --extra whisper --extra claude # NOSONAR: uv.lock pins nvidia-ml-py3==7.352.0, which has no wheel and must be built from its source distribution.
...
RUN uv sync --frozen --no-dev --extra server --extra scoring --extra whisper --extra claude # NOSONAR: the local Songmaker project is installed editable and has no wheel; third-party dependencies are frozen in uv.lock.
