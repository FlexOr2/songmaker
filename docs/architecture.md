...
| PUT | `/api/settings/cowriter` | admin | Persist co-writer provider, model, and history-tail budget. When the provider or model changes, requires that provider to be `configured` for the co-writer surface and validates the model against its live catalog; a budget-only save is never blocked by a temporarily unreachable provider or catalog. |
...
| PUT | `/api/settings/judge` | admin | Persist judge provider and model. When the provider or model changes, requires that provider to be `configured` for the judge surface and validates the model against its live catalog. |
