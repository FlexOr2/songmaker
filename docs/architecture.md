...
Prometheus mounts the containing `monitoring` directory as its rules directory, so an automatic deploy sees the rule file replaced by Git; when `monitoring/alert.rules.yml` changed from the recorded deployed SHA, `auto-deploy.sh` posts to Prometheus' loopback-only reload endpoint and compares `/api/v1/rules`' loaded alert-rule count with the rule-file count. A reload, API, or count failure is logged after the deploy is booked and does not increment its failure counter.
