...
1. **Read HEAD, fetch, read remote HEAD.** `git rev-parse HEAD` (works detached too), `git fetch origin main`, `git rev-parse origin/main`. Neither read depends on which branch is checked out, so both happen before the branch guard.
...
3. **On the deploy branch?** `git symbolic-ref --short HEAD` must equal the configured deploy branch (`main` by default). A detached HEAD or the operator sitting on an experiment branch stops here with a loud (`prio=err`) journal line and touches nothing — reached only once step 2 has established that something is actually pending.
...
The lock file lives at a fixed path inside the checkout itself (`$REPO_ROOT/.git/songmaker-autodeploy.lock`), not under a systemd-managed `RuntimeDirectory` — a unit-scoped runtime dir would give the systemd-run tick and a manual shell run of this same script two different lock inodes, defeating the mutual exclusion the lock exists for. The failure counter, the busy-deferral counter, and `deployed.sha` live under systemd's `StateDirectory` (`/var/lib/songmaker-autodeploy`) when running under the unit, and fall back to that same `.git`-relative path (not `/var/tmp`) for a manual run outside systemd, so a manual invocation shares the exact state a unit run would have used instead of starting fresh every time. A corrupted counter or `deployed.sha` file resets/re-verifies itself with a loud journal line instead of silently misbehaving.
