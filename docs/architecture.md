...
Frequent studio actions (theme toggle, pick/keep, playlist reorder/remove, new song/playlist, playlist-picker add) share the `[data-hitbox='frequent']` primitive in `frontend/src/lib/styles/hitbox.ts`. The visible glyph or inset face stays compact; the control's hitbox is 24×24px on a fine pointer and 44×44px when any pointer is coarse (including hybrid mouse+touch devices). PlayerBar and SharedPlayer are out of this primitive's scope. The 320px–768px shell keeps Logout in view by tightening header tracking; generation cards wrap pick/keep onto their own row so seed text does not paint under the rating. The header is 46px so a 44px theme hitbox stays inside the 2px bottom border.
