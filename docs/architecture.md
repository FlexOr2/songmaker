...
The lyrics column (`NowPlayingLyrics.svelte`) follows playback once the playing take carries `whisper_cues` (#45, contract confirmed on #52, word timestamps added on #142). `utils/lyrics-align.ts#alignLyricsToCues` takes one of two paths, chosen by what the take was scored with. **Word path** — a take scored with word timestamps carries a word stream (`cue.words`); lyric lines are walked in order and each takes the best-matching run of still-unconsumed words, so its interval is that line's own first…last sung word and the cursor never moves backwards. A line whose rendition does not start within `WORD_STREAM_LOOKAHEAD` (24) words of the previous match counts as not sung. **Cue window fallback** — a take scored before word timestamps carries only segment cues, and a segment follows breathing pauses rather than line breaks (the Nachtstrom take: 33 segments over 56 lines), so cues are walked in playback order and each takes the best-matching run of up to `MAX_WINDOW_LINES` (3) still-unconsumed lines, whose intervals split the cue span in proportion to their normalised length; a one-line window is exactly the cue's own `[start, end)`. Both paths share one accept rule: text is normalised by `utils/lyrics-normalize.ts`'s token rules, lines are split on `/\r?\n/` and blank plus `[section]`-marker lines never align, the winner must clear `MIN_RATIO` (0.72), and it must beat every rival by `AMBIGUITY_MARGIN` (0.12) — a rival being a candidate that neither overlaps the winner nor merely echoes it (its text contains the winner's or is contained in it), so a repeated chorus line lights at each repeat instead of being blocked by its own echo. Anything short of that leaves the line dark; a missed highlight is a gap, a wrong one is a lie. Similarity is `utils/sequence-matcher.ts`'s `SequenceMatcher`, a faithful TypeScript port of Python's `difflib.SequenceMatcher` (including the ≥200-char autojunk popular-element filter). `scripts/lyric_alignment_golden.py` is the reference: it holds both the Python-computed ratios and a Python reference implementation of both alignment paths, and `lyrics-align.fixtures.json` pins the TypeScript side to its intervals fixture by fixture, including the cases that must stay dark. `activeLyricLineIndex` highlights the line covering `audioPlayer.currentTime` and nothing in a gap, auto-scrolling the active line into view (instant under `prefers-reduced-motion`, smooth otherwise). Cues and transcript come from the take resolved against `songList` (`playingGeneration`, not `info`, per the #45 amendment), so a thin library-pool item stays on static lyrics until `ensureGenerationsLoaded` fills in its real `whisper_cues`; a take with `whisper_text` but no cues at all (scored before #44 landed) shows a "Re-score this take to follow the lyrics." hint alongside the static text. `SharedCollection.svelte` never passes cues to `NowPlayingFrame`, so the public share surface always renders the static fallback.
