...
| Components | Editor (`components/editor/`: `EditorHeader`, `SongMenu`, `RecipeChips`, `RecipePanel`, `WriteColumn`, `TakeStrip`, `TakesList`, `TakeMenu`, `EditorSheet`), PlayerBar, LibraryWall, CollectionHeader/Menu, shell/Rail, take inspector (`GenerationView`), CoWriterPanel, etc. | `src/lib/components/` |
