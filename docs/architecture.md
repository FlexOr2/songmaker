...
Two independent consecutive-tick counters distinguish "this is actually broken" from "this is just busy": a hard refusal (wrong branch, dirty tree, diverged), a fail-closed DB check, or a `pull`/`build`/`up` failure bumps the **failure** counter — only the Nth in a row (default 3, ~6 minutes) escalates to an emphatic `prio=err` "N ticks in a row deferred/failed, reason: …" line. A deferral because jobs are active (before or after the build) bumps a separate **busy-deferral** counter instead — a normal generation queue or an hours-long `lora_training` job legitimately keeps jobs active for a long time, so this one needs a much higher threshold (default 30 ticks, ~1h) and a quieter `prio=warning` "deploy pending for ~1h, N jobs still active" line, not a page-worthy error. Both counters reset only on an actual deploy (`record_success`) or a genuine "nothing to deploy" tick (step 2) — never on each other, so a run of busy deferrals doesn't mask a real failure and vice versa.
