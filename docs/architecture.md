...
The lyrics column (`NowPlayingLyrics.svelte`) follows playback once the playing take carries `whisper_cues` (#45, contract confirmed on #52): `utils/lyrics-align.ts#alignLyricsToCues` runs `greedy_monotone` — cues (normalised the same way as `utils/lyrics-normalize.ts`'s token rules, applied per line) are assigned in playback-time order to the best still-unassigned, non-blank, non-`[section]`-marker lyric line (lines split on `/\r?\n/`), never revisiting an earlier line, and a cue is skipped outright below `MIN_RATIO` (0.72) or when its best match and the best differently-worded competitor score within `AMBIGUITY_MARGIN` (0.12) of each other — an identically-worded repeated line (a chorus) is never its own competitor. Similarity is `utils/sequence-matcher.ts`'s `SequenceMatcher`, a faithful TypeScript port of Python's `difflib.SequenceMatcher` (including the ≥200-char autojunk popular-element filter), golden-tested bit-for-bit against `scripts/lyric_alignment_golden.py`'s Python-computed ratios on invented, already-normalised fixture pairs (`lyrics-align.fixtures.json`). Each matched line's interval is exactly its one assigned cue's half-open `[start, end)`; `activeLyricLineIndex` highlights the line covering `audioPlayer.currentTime` and nothing in a gap between cues, auto-scrolling the active line into view (instant under `prefers-reduced-motion`, smooth otherwise). Cues and transcript come from the take resolved against `songList` (`playingGeneration`, not `info`, per the #45 amendment), so a thin library-pool item stays on static lyrics until `ensureGenerationsLoaded` fills in its real `whisper_cues`; a take with `whisper_text` but no cues yet (scored before #44 landed) shows a "Re-score this take to follow the lyrics." hint alongside the static text. `SharedCollection.svelte` never passes cues to `NowPlayingFrame`, so the public share surface always renders the static fallback.
