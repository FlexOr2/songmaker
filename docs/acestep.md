...
The script copies the unit into `/etc/systemd/system/`, reloads systemd, and enables + starts it; rerunning it is a no-op. It only installs the boot hook — it does not rebuild or otherwise touch the live containers.
