...
On the songmaker side, `acestep_engine.models.ResultItem`/`AceStepResult` parse the two fields (mirroring `error`/`status_message`), and `StoredGenerationParams.delivered_batch_size` is where a reduction would land on `Generation.generation_params` — read alongside the inherited `batch_size` (the request). `frontend/src/lib/constants/now-playing.ts:takeBatchReductionLabel()` turns the pair into a `⚠ N of M` badge on the take row (`TakesList.svelte`) whenever they're both present and differ; the raw `delivered_batch_size` value also falls into the take's "Other" recipe group for free, the same way any generation_params key the registry doesn't name does.

**Known gap:** this chain stops at the worker boundary. `src/acestep_worker/wrapper.py`'s `default_generate_runner` builds `GenerationTaskResult` (`src/acestep_worker/models.py`) from `AceStepClient.generate()`'s `AceStepResult` but does not yet copy `delivered_batch_size` onto it, so `GenerationTaskResultDTO.delivered_batch_size` on the songmaker_cli side (already wired, defaults to `None`) never receives a real value from a running deployment — no `Generation` has shown the badge yet. Closing it is a two-field pass-through in those two worker files, deliberately left to whichever lane owns `acestep_worker` next (issue #211's build lane was scoped out of that package while #205 was active on `subprocess_runner.py`).
