...
On the songmaker side, `acestep_engine.models.ResultItem`/`AceStepResult` parse the two fields (mirroring `error`/`status_message`) — real, reachable data the moment a caller of `acestep_engine.client` asks for it. `StoredGenerationParams.batch_size` now also persists the *requested* side onto `Generation.generation_params` whenever it isn't the trivial default of 1 (previously dropped silently, even though `ParamControls`/`acestep-param-fields.ts` already expose it as an editable knob), so the request half of "2 asked, 1 delivered" is visible today.

**Known gap — the delivered side is not wired end to end.** `src/songmaker_cli/scheduler.py`'s `GenerationTaskResultDTO` is intentionally kept field-identical to `acestep_worker.models.GenerationTaskResult` (`tests/test_scheduler.py::test_dto_keys_match_worker_model_fields` enforces it as a drift guard), and `acestep_worker/wrapper.py`'s `default_generate_runner` does not yet copy `AceStepResult.delivered_batch_size` onto that worker-side model. Closing the loop needs, together, in `src/acestep_worker/` (models.py + wrapper.py) and `src/songmaker_cli/` (the DTO field, a `StoredGenerationParams.delivered_batch_size` field, and threading it through `post_process_generation`/`_persist_generation_row`) plus a small take-row badge reading both fields off `generation_params` — deliberately left to whichever lane next owns `acestep_worker` (issue #211's build lane was scoped out of that package while #205 was active on `subprocess_runner.py`). Until then, no `Generation` shows a delivered-batch reduction; the fork and `acestep_engine` sides are ready and tested for the day that lands.
