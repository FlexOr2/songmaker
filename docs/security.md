...
After a successful recreate, the tick runs `docker image prune --force --filter until=48h`: dangling images only, so it does not remove tagged base images, stopped E2E stacks, or images belonging to another project. Before recreating, it tags every running Compose service image as `songmaker-<service>:previous`; the next deploy moves that fixed tag forward, retaining exactly one rollback generation. To roll back a service, retag `songmaker-<service>:previous` to the image reference that service uses in `docker-compose.yml`, then run `docker compose up -d <service>`. Both cleanup commands are bounded by a 600-second timeout; an error or timeout is logged and increments the existing consecutive-failure counter, while the completed deployment remains successful. The builder cleanup deliberately uses `docker builder prune --all --force --filter until=48h`, preserving cache used in the last 48 hours but allowing a quiet host's next rebuild to take its normal cold-cache 8–15 minutes.
