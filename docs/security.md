...
Static `_app/` build assets, the health check, and the static PWA root assets (`/health`, `/manifest.webmanifest`, `/robots.txt`, `/favicon.svg`, `/icon-*.png`, `/service-worker.js`) are exempt from this budget — they're fetched by infrastructure, the browser, and the service worker outside of user-driven navigation and would otherwise crowd out real `/api/*` calls from the same IP. No `/api/*` path is exempt.
