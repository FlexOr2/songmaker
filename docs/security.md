...
- **CLI backend, every call shape**: `_build_mcp_cli_cmd()` (the co-writer, MCP tools attached) and `_build_cli_cmd()` (the legacy `/songs/{id}/chat` endpoint and the lyrical-coherence judge, no tools at all) both apply `--tools ""` (removes the CLI's entire built-in tool set, so a tool a future Claude Code version ships cannot be called even though nobody here has heard of it), `--setting-sources ""` (drops the mounted `~/.claude/settings.json`, whose `permissions.allow` and `defaultMode` would otherwise decide what a session may do), `--strict-mcp-config` (ignores MCP servers configured anywhere but in the temporary config we write, when one is attached at all), and `--disable-slash-commands` (the CLI otherwise still resolves its own slash commands and skills from a prompt beginning with `/`; our own prompts never do, but this closes the channel rather than relying on that). The co-writer additionally gets `--allowedTools mcp__songmaker__*`, pre-approving our own MCP tools so the non-interactive session never waits for a permission answer nobody is there to give. No `--disallowedTools` list is kept, and `--permission-mode bypassPermissions` is gone from both — with nothing but the allowlisted tools present, there is nothing left to bypass.
- **Tool-surface verification, one gate per call shape, not per caller**: both command builders are reached only through `_call_cli()`/`_acall_cli()` and the two MCP-attached entry points, and every one of those calls its gate before building anything — so `chat_api.py`'s legacy endpoint and the lyrical-coherence judge are covered by the same gate the co-writer uses, without either having called it themselves (#351). The CLI is a bind-mounted binary that updates itself, so before every such call `verify_cli_tool_surface()` (co-writer) or `verify_no_builtin_cli_tools()`/`averify_no_builtin_cli_tools()` (every tool-free call) starts a session with exactly the flags above and reads the tool names — and the `slash_commands` list — out of the CLI's own `system` init event (`subtype` checked too, not just `type`). The co-writer's probe attaches the real `--mcp-config` and requires the reported tool set to equal, exactly, the eleven `mcp__songmaker__*` names in `mcp_server/server.py` — not merely be a subset of them, so a tool going missing is caught the same way an extra one is. The tool-free probe requires no tool and no slash command at all, and deliberately never attaches `--mcp-config`: doing so would need the `mcp` extra and a reachable database, neither available in the scoring-worker container (confirmed live: a failed MCP connection reports zero tools, not eleven), and the tool-free command line never attaches one either, so the check matches what actually runs. A mismatch raises `CliToolSurfaceError` and only that call is refused, not the whole server. The verdict is cached per binary build (resolved path, size, mtime), so a self-update is verified again instead of inheriting the old answer; a probe *failure* is also cached, for `CLAUDE_CLI_TOOL_SURFACE_FAILURE_CACHE_SECONDS`, so a struggling CLI or database does not make every concurrent caller each wait out the probe's own timeout, while a real repair still takes effect on the next request rather than staying failed for the life of the success cache. The gate resolves the CLI's symlink once, at probe time, and hands that literal path back for the real turn to run — so a self-update landing between the probe and the turn cannot execute a build that was never checked.
