...
Idempotent — skips generations that already exist (matched by mp3_path).
"""
...
def migrate_filesystem(
    ...
    counts = {
        "albums": 0, "songs": 0, "versions": 0,
        "generations": 0, "scores": 0, "ratings": 0, "skipped": 0,
    }
    ...
    for scan in scans:
        ...
        _ensure_album(session, scan.album_name, scan.meta)
        if is_new:
        ...
        for mp3 in scan.mp3s:
            ...
            existing = session.query(Generation).filter_by(mp3_path=mp3_path).first()
            if existing:
                counts["skipped"] += 1
                continue
            ...
            gen_num = extract_version_number(stem) or 1
            _, title = parse_track_title(stem)

            song = song_cache.get(base)
            if not song:
                song = _ensure_song(session, base, title, scan.album_name, scan.lyrics_dir)
                song_cache[base] = song
                counts["songs"] += 1

            gen = _create_generation(session, song, mp3, scan.mp3_base, gen_num)
            counts["generations"] += 1

            snapshot_path = mp3.with_suffix(".md")
            sc = _import_scores_and_rating(session, gen, snapshot_path)
            counts["scores"] += sc["scores"]
            counts["ratings"] += sc["ratings"]

    session.commit()
    log.info(
        "Migration: %d albums, %d songs, %d generations, %d scores, %d ratings (%d skipped)",
        counts["albums"], counts["songs"], counts["generations"],
        counts["scores"], counts["ratings"], counts["skipped"],
    )
    return counts


def _ensure_album(session: Session, album_name: str, meta) -> Album:
...
def _ensure_song(
    session: Session, base_stem: str, title: str, album_id: str, lyrics_dir: Path,
) -> Song:
    existing = session.query(Song).filter_by(album_id=album_id, title=title).first()
    if existing:
        return existing
    ...
    song = Song(title=title, album_id=album_id, track_number=track_number)
    session.add(song)
    session.flush()
...
def _read_lyrics_source(stem: str, lyrics_dir: Path) -> tuple[str, str, int, int, str]:
...
def _create_generation(
    session: Session, song: Song, mp3: Path, mp3_base: str, gen_num: int,
) -> Generation:
    snapshot_path = mp3.with_suffix(".md")
    ...
    gen = Generation(
        song_id=song.id,
        version_id=song.latest_version.id if song.latest_version else None,
...
def _import_scores_and_rating(
    session: Session, gen: Generation, snapshot_path: Path,
) -> dict[str, int]:
    ...
    score_values = {k: v for k, v in score_data.items() if k in SCORE_KEYS}
    if score_values:
        score = Score(generation_id=gen.id, scorer="batch", value=score_values)
        session.add(score)
        counts["scores"] = 1
